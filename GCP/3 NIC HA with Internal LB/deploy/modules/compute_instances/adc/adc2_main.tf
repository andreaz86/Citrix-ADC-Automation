resource "google_compute_instance" "vm_adc02" {
  name         = "${var.vmname_prefix}${var.adc_vm.adc02.name}"
  machine_type = var.adc_vm.adc02.vmtype
  zone         = var.adc_vm.adc02.zone
  tags         = ["ssh", "adc", "https"]
  ...
  boot_disk {
    ...
    auto_delete = true
    initialize_params {
      image = var.adc_vm.adc02.vmimage
    }
  }
  ...
  network_interface {
    ...
    network_ip = var.adc_vm.adc02.client_ip
  }

  network_interface {
    subnetwork = var.mgmt_subnetself_id
    network_ip = var.adc_vm.adc02.mgmt_ip
    access_config {
    }
  }
  ...
  network_interface {
    ...
    network_ip = var.adc_vm.adc02.server_ip
  }
